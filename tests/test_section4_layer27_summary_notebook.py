...
def test_summary_notebook_preserves_data_and_bands(directory, monkeypatch):
    ...
    a, b, c = fig.axes
    assert [len(ax.lines) for ax in fig.axes] == [2, 3, 5]
    assert a.get_xscale() == a.get_yscale() == 'linear'
    assert b.get_yscale() == c.get_yscale() == 'log'
    assert len(b.collections) == 3
    assert isinstance(b.collections[0], LineCollection)
    assert scope['DRIFT_CONDITION'] == 'm4_b1_g3'
    assert len(c.collections) == 5
    ...
    for line, direction in zip(b.lines, ['null', 'sensitive', 'random']):
    ...
    null = [r['native_error'] for r in drift if r['direction']=='null']
    for segment, moment in zip(b.collections[0].get_segments(), null):
        np.testing.assert_allclose(segment[:, 1],
                                   [max(moment['mean']-moment['std'], b.get_ylim()[0]),
                                    moment['mean']+moment['std']])
